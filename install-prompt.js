 ...
 *     replays it.
 *   - iPhone: the native app has been on the App Store since 2026-09-10, so
 *     the card offers that instead of the old Share > Add to Home Screen
 *     instructions. It is strictly the better install (alerts arrive in the
 *     background), and any iOS browser can open a store link, so this path no
 *     longer requires Safari. Safari also shows Apple's own Smart App Banner
 *     from the apple-itunes-app meta tag; this card is the second touch, ten
 *     seconds in, for readers who scrolled past it.
 *
...
(function () {
  ...
  // Mirrors APP_STORE_URL in /core/config.js. This file is a classic script
  // with no imports, so the value is repeated here and a test pins the two.
  var APP_STORE_URL = 'https://apps.apple.com/app/bowcast-rainbow-forecast/id6805224715';
  ...
  // Firefox): stay silent.
  if (!chromiumCapable && !isIOS) return;
  ...
  // The transparent source preserves the supplied mark while allowing each
  // page temperature to remain visible through and around the arcs.
  var ARC_SVG = '<img src="/icons/bowcast-mark.png" alt="" aria-hidden="true">';
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '.bc-install-mark img{width:100%;height:100%;display:block;object-fit:contain;}',
      ...
      '  display:inline-flex;align-items:center;text-decoration:none;box-sizing:border-box;}',
      ...
      '.bc-install-close{position:absolute;top:3px;right:3px;width:44px;height:44px;',
  ...
  function buildCard(variant) {
    ...
    var title = isIosVariant
      ? 'Bowcast is on the App Store'
      : 'Add Bowcast to your home screen';
    el.setAttribute('aria-label', title);

    var bodyLine = isIosVariant
      ? 'Alerts that arrive while the app is closed, and a forecast that follows you. Free, no account.'
    ...
    var actions = isIosVariant
      ? '<button class="bc-install-btn bc-install-ghost" type="button" data-bc="dismiss">Not now</button>'
        + '<a class="bc-install-btn bc-install-primary" href="' + APP_STORE_URL + '" rel="noopener" data-bc="store">Get the app</a>'
      : '<button class="bc-install-btn bc-install-ghost" type="button" data-bc="dismiss">Not now</button>'
    ...
    el.innerHTML =
      ...
      +     '<p class="bc-install-title">' + title + '</p>'
    ...
    el.addEventListener('click', function (event) {
      ...
      if (!trigger) return;
      var action = trigger.getAttribute('data-bc');
      if (action === 'add') addApp();
      // The store link navigates on its own; remember the choice so the card
      // does not come back for a reader who already went to install.
      else if (action === 'store') { lsSet(LS_DONE, '1'); teardown(); }
      else dismiss();
    });
