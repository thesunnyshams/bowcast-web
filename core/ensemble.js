...
export function normalizeEnsembleResult(location, result) {
  ...
  const members = suffixes.map((sfx) => ({
    ...
    // Wind and temperature are occurrence gates in scoring (Liu et al. saw no
    // rainbows at Beaufort >= 8 or below 8 C). Taking them per member keeps
    // the headline estimate on the same footing as the deterministic score,
    // rather than borrowing one deterministic value for all 40 atmospheres.
    tempC: hourly['temperature_2m' + sfx] ?? [],
    windKmh: hourly['wind_speed_10m' + sfx] ?? [],
  }));
...
export async function fetchEnsembles(locations, model, timezone = 'auto', days = 7) {
  ...
  url.searchParams.set('hourly', 'precipitation,rain,snowfall,cloud_cover,direct_normal_irradiance,temperature_2m,wind_speed_10m');
