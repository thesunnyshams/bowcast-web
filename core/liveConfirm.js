 ...
 */
export function liveVerdict({ sunElevation, radar, sky, sunlitForecast }) {
  ...
  const sunForecast = (sunlitForecast ?? 0) >= FORECAST_SUN_MIN;
  if (!sunObserved && !sunForecast) {
    return { live: false, reason: 'sun not shining on the shaft' };
  }
  return {
    live: true,
    tier: sunObserved ? 'confirmed' : 'likely', // satellite-observed sun vs forecast sun
...
export async function confirmNow(city) {
  ...
  const v = liveVerdict({ sunElevation: pos.elevation, radar, sky, sunlitForecast: city.sunlitNow });
  return {
    ...v,
