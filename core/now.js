...
function scoreCityNow(city, forecast, ens, nowEpoch, nowMs = Date.now()) {
  ...
  return {
    ...
    // The Now board's geometry column reports the sun as well as the chance,
    // because a bow is impossible below the horizon or above 42 degrees however
    // the rain behaves. Carried here rather than recomputed in the browser, so
    // the column and the ranking describe the same instant.
    sunElevation: Math.round(currentSun.elevation),
