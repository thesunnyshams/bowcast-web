...
import { keepLandPoints } from './land.js';
import {
...
export function alignEnsembleMembers(ensemble, hours) {
  ...
  return ensemble.members.map((member) => ({
    ...
    // Scoring 3.8 gates each member on its own wind and temperature. Both are
    // optional per model, so they are carried with `?.` like rainMm. Forgetting
    // a signal here silently disables the factor that consumes it, which is
    // exactly how it stays broken: scoring keeps returning a plausible number.
    tempC: hours.map((hour) => member.tempC?.[idxByEpoch.get(hour.epoch)] ?? null),
    windKmh: hours.map((hour) => member.windKmh?.[idxByEpoch.get(hour.epoch)] ?? null),
  }));
}
...
export function sliceEnsembleMembers(members, start, end) {
  ...
  return members.map((member) => ({
    ...
    tempC: member.tempC?.slice(start, end) ?? [],
    windKmh: member.windKmh?.slice(start, end) ?? [],
  }));
}
...
export async function computeLikelihood(locations, {
  ...
  const locationsResult = forecasts.map((forecast, idx) => {
    ...
    return {
      ...
      lat: forecast.lat,
      lon: forecast.lon,
      // true over land, false over water, null when the DEM was unreachable
      // or the caller never asked. Surfaces filter through keepLandPoints.
      land: forecast.land ?? null,
      ...
      bow: bowFor(today),
      reason: today.reason,
      reasonParts: today.reasonParts,
      hourly: today.hourly, // full hourly detail is today-only to keep the payload lean
      days: days.map((d) => ({
        date: d.date,
        weekday: d.weekday,
        // Seconds, equal to intervalStartEpoch: the instant whose local
        // calendar date (in this payload's `timezone`) is this day. Pair with
        // Intl.DateTimeFormat(locale, { weekday, timeZone }) to derive a
        // weekday label in any locale; always falls on the same calendar date
        // the server used for `weekday` above, even on day 0 when the first
        // published hour is partway through today rather than at midnight.
        epoch: d.intervalStartEpoch,
        ...
        bow: bowFor(d),
        reason: d.reason,
        reasonParts: d.reasonParts,
      })),
    };
  });

  // Week outlook: the single best town/day in the region, so the UI has
  // something to point at even when today is a wall of zeros. Ranked over the
  // same points the surfaces draw, or the banner ends up naming a ring point
  // the map has already dropped for sitting on open water.
  let outlook = null;
  let outlookRank = 0;
  keepLandPoints(locationsResult).forEach((loc) => {
