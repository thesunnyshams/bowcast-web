...
async function latestFrame(fetchImpl) {
  const now = Date.now();
...
const nowcastCache = new Map(); // key -> Promise
 ...
 *                     quality?, reason? }>}
 */
export function radarNowcast(lat, lon, sunAzimuthDeg, { fetchImpl, zoom = 9 } = {}) {
  if (typeof document === 'undefined') return Promise.resolve({ available: false });
  const key = `${lat.toFixed(3)},${lon.toFixed(3)},${Math.round(sunAzimuthDeg / 15) * 15}`;
  if (!nowcastCache.has(key)) {
    const promise = (async () => {
      const frame = await latestFrame(fetchImpl);
      ...
      for (const p of pts) {
        ...
        if (!ctxByTile.has(tk)) {
          ...
          ctxByTile.set(tk, loadTileCtx(url).catch(() => null));
        }
        const ctx = await ctxByTile.get(tk);
        if (!ctx) { samples.push({ km: p.km, rain: false, level: 0 }); continue; }
        const [r, g, b, a] = ctx.getImageData(p.px, p.py, 1, 1).data;
        samples.push({ km: p.km, ...classifyPixel(r, g, b, a) });
      }
      return { available: true, ageMin: Math.max(0, Math.round((Date.now() / 1000 - frame.time) / 60)), ...summarizeShaft(samples) };
    })();
    promise.catch(() => nowcastCache.delete(key));
    nowcastCache.set(key, promise);
  }
  return nowcastCache.get(key);
}
