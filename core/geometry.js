 ...
 *
 * @param {number} epochSec inclusive start of the forecast interval (unix seconds)
 * @param {number} lat observer latitude
 * @param {number} lon observer longitude
 * @param {number} [endEpochSec=epochSec+3600] exclusive interval end
 ...
 */
export function bowGeometry(epochSec, lat, lon, endEpochSec = epochSec + 3600) {
  if (!Number.isFinite(epochSec) || !Number.isFinite(endEpochSec) || endEpochSec <= epochSec) return null;
  let first = null;
  let last = null;
  let eligibleMinutes = 0;
  for (let t = epochSec; t < endEpochSec; t += 60) {
  ...
  return {
    startEpoch: first,
    endEpoch: Math.min(last + 60, endEpochSec),
    intervalStartEpoch: epochSec,
    intervalEndEpoch: endEpochSec,
    eligibleMinutes,
