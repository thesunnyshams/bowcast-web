import { PushValidationError, unregisterToken } from '../_push/service.js';
...
export function createUnregisterHandler({
  initialize = ensurePushInitialized,
  unregister = unregisterToken,
  verifyAppCheck = verifyPushAppCheckRequest,
} = {}) {
  return async function unregisterHandler(req, res) {
    const observation = observeServerless(req, res, '/api/push/unregister');
    ...
    try {
      ...
      await initialize();
      await unregister(req.body?.token);
      observation.done(200, { appCheck: appCheck?.status || 'unknown' });
      ...
      return res.status(503).json({ error: 'push unregistration temporarily unavailable' });
    }
  };
}

export default createUnregisterHandler();
