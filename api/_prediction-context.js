...
/**
 * Whether an unverified report may buy a fresh forecast at submission time.
 *
 * Off unless asked for, because the caller chooses the coordinates and pays
 * nothing. `options.refreshForecast` lets a caller (and the tests) decide
 * explicitly; otherwise it takes the environment.
 */
function refreshOnSubmitEnabled(options = {}) {
  if (options.refreshForecast != null) return Boolean(options.refreshForecast);
  return process.env.SIGHTING_FORECAST_REFRESH === '1';
}
...
/**
 * A sighting is always reported for the interval the observer is standing in:
 * buildSightingReport() in public/app.js takes the interval containing now, or
 * the nearest one to it. A token for an hour half a day out can never be
 * redeemed, and at roughly 500 bytes each they were 82% of the response
 * (265 KB of 324 KB for nine locations). Sign only the hours near now, wide
 * enough to cover a 15-minute cached response, a clock a little off, and the
 * nearest-interval fallback.
 */
const SIGNED_WINDOW_SECONDS = 4 * 60 * 60;

function withinSigningWindow(hour, nowSec) {
  const from = hour.validFromEpoch ?? hour.epoch;
  const to = hour.validToEpoch ?? hour.epoch;
  if (!Number.isFinite(from) || !Number.isFinite(to)) return false;
  return to >= nowSec - SIGNED_WINDOW_SECONDS && from <= nowSec + SIGNED_WINDOW_SECONDS;
}
  ...
  return {
    ...
    locations: result.locations.map((location) => ({
      ...
      hourly: (location.hourly || []).map((hour) => {
        if (!withinSigningWindow(hour, nowSec)) return hour;
...
export async function verifyOrRefreshSightingRecord(
  ...
  // An unverified token used to fall through to a fresh computeLikelihood at
  // caller-chosen coordinates. Sightings are POSTed, so nothing is edge-cached
  // and that was one guaranteed upstream fetch per anonymous request: the
  // cheapest way to spend the forecast budget from outside. 1.0 refuses it and
  // files the report with client-supplied context, which `trustSightingRecord`
  // has already marked `client`. Re-enable with SIGHTING_FORECAST_REFRESH=1
  // once report volume is worth the exposure.
  if (!refreshOnSubmitEnabled(options)) return verified;
