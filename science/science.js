...
// CSS custom properties, so every hand-drawn chart re-inks itself when the
// theme flips (SVG presentation attributes accept var()).
const C = { ink: 'var(--ink)', muted: 'var(--muted)', line: 'var(--line)', paper2: 'var(--paper-2)',
  red: 'var(--red)', amber: 'var(--amber)', teal: 'var(--teal)', violet: 'var(--violet)', accent: 'var(--accent)' };
...
function drawBow(sun) {
  ...
  sky += `<rect x="0" y="0" width="${W}" height="${horizon}" fill="var(--paper-2)"/>`;
